...
> **PI decision memo (phase-by-phase, restart-ready): `docs/decisions/DESIGN_DECISION_MEMO.md`.**

**Phase / Stage:** Stage 6 — monomer counter-screen rejected all top-6 T2 (monomer-drift; validates §2). **PI pivot = protofibril-target model.** Done so far: structural ground-truth (9CO4/7Q4B core 9-42, flexible N-term 1-8 = B3; 8QN7 CAA N-term 1-38 ordered = B4); feasibility test (WT Fv + 3× free Aβ42) FAILED — Aβ self-aggregates, Fv unbound. **PROTOFIBRIL-TARGET MODEL (S2-3A) BUILT + WT-VALIDATED.** PyRosetta dock (`scripts/stage6_select/dock_fv_protofibril.py` seed/dock; `slurm/stage6_dock.sbatch`), one identical DockMCM+InterfaceAnalyzer protocol for all forms. WT dG_separated: **monomer (free Aβ1-16) −27.56 REU** (49 res, full wrap) > **protofibril 9CO4 −8.62** (9 res, restricted accessible N-term) > **CAA 8QN7-stack OCCLUDED** (seed 205 clashes; N-term buried). → reproduces B3/B4 AND shows the protofibril preference is **avidity** not monovalent affinity (monomer binds *tighter* monovalently) — mechanistically explains the T2 monomer-drift and validates §2. Artefacts: `results/stage6/protofibril_model/wt_validation.json`; 8QN7 stack `data/interim/antigen/8qn7_stack.pdb` (NCS op2/op5). **NEXT: re-screen variants** — dock each variant Fv onto 9CO4 + monomer (same protocol), select on margin = Δ-protofibril − Δ-monomer (improve restricted protofibril interface WITHOUT improving the monomer wrap). Then T1/T2 re-rank; pursue avidity (Stage 7) + build lecam-dev. Caveats in wt_validation.json (magnitudes soft; monovalent only). See `results/stage6/PROTOFIBRIL_MODEL_NOTES.md`. Also owed: lecam-dev (developability/humanness); avidity (Stage 7) is the other lever.
