...
**Phase / Stage:** Stage 6 — monomer counter-screen rejected all top-6 T2 (monomer-drift; validates §2). **PI pivot = protofibril-target model.** Done so far: structural ground-truth (9CO4/7Q4B core 9-42, flexible N-term 1-8 = B3; 8QN7 CAA N-term 1-38 ordered = B4); feasibility test (WT Fv + 3× free Aβ42) FAILED — Aβ self-aggregates, Fv unbound. **PyRosetta local-docking PIPELINE BUILT & VALIDATED** (`scripts/stage6_select/dock_fv_protofibril.py` seed/dock; `slurm/stage6_dock.sbatch`): clash-free seed (0 clashes) → DockMCM (receptor rigid) + InterfaceAnalyzer → WT favourable on 9CO4 (dG −5.69 REU, 459Å²). Scale-up = job **11950541** (20 decoys; check `results/stage6/dock_9co4_wt/dock_report.json`). **CONSISTENCY RULE:** raw Boltz complexes are not Rosetta-relaxed (InterfaceAnalyzer → +257 REU) — score target & ALL counter-targets through the SAME dock+refine protocol. **NEXT:** (1) same dock on the **monomer** (WT Fv + free Aβ1-16) for a comparable baseline; (2) **CAA control** — build the **8QN7 fibril stack** (deposited cif = 1 chain; apply helical symmetry so the ordered N-term is packed) then dock → expect OCCLUDED (B4); (3) validate WT (favourable 9CO4, occluded 8QN7); (4) re-screen variants on the protofibril-vs-monomer/CAA margin. Caveat: monovalent docking = conformational/steric (B3/B4) selectivity only, NOT avidity (Stage 7). See `results/stage6/PROTOFIBRIL_MODEL_NOTES.md`. Also owed: lecam-dev (developability/humanness); avidity (Stage 7) is the other lever.
