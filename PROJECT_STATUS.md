...
**Last updated:** 2026-06-01  ·  **Updated by:** Hamid (with Claude)  ·  **Phase:** Phase 0 (done) → Stage 1 (in progress)
...
Phase 0 scaffold **executed** on Frontenac (`00_init.sh`): full repo tree + git repo + coordination layer + `db/schema.sql`. **Stage 1 research deliverables complete:** lecanemab Fv sourced & cross-verified (KEGG/WHO-INN ↔ BioArctic patent), antigen templates identified, B1–B7 source sentences extracted. `lecam` conda env build in progress (conda-forge/bioconda; CC wheelhouse incompatible with login-node glibc 2.28 — see D-007). **Pending env:** DuckDB ledger init + ANARCI numbering.
Immediate next action: finish `lecam` env → init ledger → ANARCI-number the Fv (IMGT/Kabat/Chothia) + Vernier annotation → push to GitHub.
...
| 1 | Inputs, Targets & Objective Lock-In | ◐ Fv+CDRs+antigens+B1–B7 done; metrics thresholds await Stage-2 baseline |
...
**Next (immediate):**
1. Set `configs/metrics.yaml` thresholds + WT references once a Stage-2 WT baseline exists (currently TODO).
2. Pick the Aβ-monomer counter-target template (OQ; e.g. 1IYT/1Z0Q/2LFM) — Stage 2.
3. Pin exact B2 (>10⁶ selectivity, R-ANA) + B5 (no-co-structure, R-ELIFE/R-REV) sentences (currently paywalled/flagged).
...
| Antigen templates | ◐ IDs identified (`data/raw/antigen/`); monomer template TBD; downloads deferred |
| `docs/sources/` extracts (B1–B7) | ◐ written; B2 + B5 exact sentences flagged (paywalled) |
