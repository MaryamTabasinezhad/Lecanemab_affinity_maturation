...
| 4 | Variant Generation (multi-track) | ◐ **T1 generated + scored**: 3 CDR-preserving framework mutants (LEC-AM-T1-0001..3), Boltz-2 Δ-ipSAE vs WT all **neutral-within-noise** (−0.08…−0.14; expected for framework changes) → status=scored. AbLIFT/Rosetta VH-VL pending `lecam-rosetta`; T2/T3/T4 not started |
| 5 | In Silico Affinity Scoring & Consensus Ranking | ◐ harness live (per-variant Boltz-2 multi-seed → Δ-ipSAE vs WT; `slurm/stage4_score_cofold.sbatch` array); ran on T1. flex_ddG/FoldX 2nd scorer pending `lecam-rosetta` |
